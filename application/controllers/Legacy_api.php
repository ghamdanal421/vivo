...
/**
 * GHMV - Android Live TV & Movie Portal App
 * ---------------------- GHMV --------------------
 * ------- Android Live TV & Movie Portal App --------
 * - Live tv channel & movie management system -
 *
 * @package     GHMV - Android Live TV & Movie Portal App
...
class Legacy_api extends CI_Controller{   
    ...
    // Ghamdan web Index 
    public function ghamdan_index($param1 = '', $param2 = ''){
        // add,edit videos or movies 
            ...
            //    $filter['is_tvseries '] = 0;
               ...
               if($category !="" && $category !=NULL){
                   ...
                   $filter['is_tvseries'] = $category == "movies" ? 0 : 1;
                   $search_string.= 'category='.$title.'&';
                   $data['category'] = $category;
                   if( $category == "movies"){
                    $hedr['page_name']      = 'movies';
                   }else {
                    $hedr['page_name']      = 'tv';
                   }
               }else {
               ...
               if($country !="" && $country !=NULL){
                   ...
                   $search_string.= 'country='.$country.'&';
                   $data['country'] = $country;
               } 
               if($genre !="" && $genre !=NULL){
                   $filter['genre'] = $genre;
                   $search_string.= 'genre='.$genre.'&';
                   $data['genre'] = $genre;
               }
               ...
               $config["base_url"]         = base_url() . "?".$search_string;
               ...
               $hedr['page_title']     = 'الرئيسة';             
   $this->load->view('web/includes/templates/header', $hedr); 
   ...
   $this->load->view('web/includes/templates/footer');
    ...
    // Ghamdan web Index 
    public function ghamdan_favorite($param1 = '', $param2 = ''){
        // add,edit videos or movies 
               ...
               $data["favorite"]   = $this->db->get_where('favorite' , array('user_id' => $user_id) )->result_array();
            
               $hedr['page_name']      = 'favorite';
               $hedr['page_title']     = 'المفضلة';             
   $this->load->view('web/includes/templates/header', $hedr); 
   $this->load->view('web/favorite', $data); 
   $this->load->view('web/includes/templates/footer');
}


    public function ghamdan_anime($param1 = '', $param2 = '', $param3 = ''){

        $data['param1']         = $param1;
        $data['param2']         = $param2;
        $data['param3']         = $param3;

        $hedr['page_name']      = 'anime';
        $hedr['page_title']     = $param1;             
$this->load->view('web/includes/templates/header', $hedr); 
        $this->load->view('web/anime', $data); 
        $this->load->view('web/includes/templates/footer');
    }
    public function ghamdan_spaces(){
        
        $hedr['page_name']      = 'spaces';
        $hedr['page_title']     = 'ترقية الحساب';             
$this->load->view('web/includes/templates/header', $hedr); 
        $this->load->view('web/spaces'); 
        $this->load->view('web/includes/templates/footer');
    }
     public function ghamdan_request( $param1 = ""){
        ...
        if ($param1 == 'add') {
            ...
            $data['uesr_name']  = $this->input->post('uesr_name');
            $data['email'] = $this->input->post('email');
            $data['name_anime'] = $this->input->post('name_anime');
            $data['season'] = $this->input->post('season');
            $data['user_id'] = $this->session->userdata('admin_is_login') != 1 ||  $this->session->userdata('user_is_login') == 1? null : $this->session->userdata('user_id');
            $this->db->insert('request', $data);
            $insert_id = $this->db->insert_id();
            $this->session->set_flashdata('success', 'تم أرسال طلبك بنجاح ');
            redirect($this->agent->referrer());
        }



        $hedr['page_name']      = 'request';
        $hedr['page_title']     = 'تقديم طلب';             
$this->load->view('web/includes/templates/header', $hedr); 
        $this->load->view('web/request'); 
        $this->load->view('web/includes/templates/footer');
    }
    public function ghamdan_contact( $param1 = ""){
        ...
        if ($param1 == 'add') {
            ...
            $data['user_ip']  = $ip;
            $data['contact_at']  =  date('Y-m-d H:i:s');
            $data['user_agent']  =  $userAgent;
            $data['referer']  = $this->input->post('url');
            $data['message']  = $this->input->post('message');
            $data['uesr_name']  = $this->input->post('uesr_name');
            $data['email'] = $this->input->post('email');
            ...
            $insert_id = $this->db->insert_id();
            $this->session->set_flashdata('success', 'تم أرسال رسالتك بنجاح ');
            redirect($this->agent->referrer());
        }



        $hedr['page_name']      = 'contact';
        $hedr['page_title']     = 'أبلاغ عن مشكلة ';             
$this->load->view('web/includes/templates/header', $hedr); 
        $this->load->view('web/contact'); 
        $this->load->view('web/includes/templates/footer');
    }
    public function ghamdan_profile($param1 = ''){

        if ($param1 != "update" && $param1 != "change_password"){
            $data['url_param'] = $param1;
        }
        ...
        if ($this->session->userdata('admin_is_login') == 1 ||  $this->session->userdata('user_is_login') == 1){
         ...
         redirect(base_url() . '#login', 'refresh');
          ...
          if ($param1 == 'update') {
              ...
              $this->common_model->clear_cache();
              $this->session->set_flashdata('success', 'تم حفظ التعديلات.');
              redirect($this->agent->referrer());
          }
          if ($param1 == 'change_password'){
              ...
              if ($current_password == $password && $new_password == $retype_new_password) {
                  ...
                  $this->session->set_flashdata('success', 'تم تغير الباسورد ورفع التعيلات');
              }
              elseif ($current_password !=$password ){
                  $this->session->set_flashdata('error', 'كلمة المرور القديمة غير صحيحة');
  
              } else {
                  $this->session->set_flashdata('error', 'كلمة المورو غير متطابقة');
              }
              redirect($this->agent->referrer());
          }
          ...
          $hedr['page_name']      = 'profile';
          $hedr['page_title']     = 'الملف الشخصي ';             
  $this->load->view('web/includes/templates/header', $hedr); 
        $this->load->view('web/profile', $data); 
        $this->load->view('web/includes/templates/footer');
    }
    // public function ghamdan_signup(){
