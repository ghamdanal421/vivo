...
class Admin extends CI_Controller {   
    ...
    function system_setting($param1 = '', $param2 = ''){
        ...
        if ($param1 == 'update') { 
             ...
             $data['value'] = $this->input->post('description_meta');
             $this->db->where('title' , 'description_meta');
             $this->db->update('config' , $data);
             $data['value'] = $this->input->post('logo_site');
             $this->db->where('title' , 'logo_site');
             $this->db->update('config' , $data);
    ...
    function backup_restore($operation = '', $type = ''){
     ...
     // live tv category
     function aps_web($param1 = '', $param2 = ''){
        // active menu session
        $this->session->unset_userdata('active_menu');
        $this->session->set_userdata('active_menu', '55');       
        
        if ($param1 == 'update') {
            $data['aps_web_address']             = $this->input->post('aps_web_address');
            $data['aps_web_html']        = $this->input->post('aps_web_html');           
            $this->db->where('aps_web_id', $param2);
            $this->db->update('aps_web', $data);
            $this->session->set_flashdata('success', 'APS  update successed.');
            redirect(base_url() . 'admin/aps_web/', 'refresh');
        }
        
        $data['page_name']      = 'aps_web';
        $data['page_title']     = 'APS WEB Management';
        $data['aps_web']    = $this->db->get('aps_web')->result_array();             
        $this->load->view('admin/index', $data);
    }
    // live tv contact
    function contact($param1 = '', $param2 = ''){
        // active menu session
        $this->session->unset_userdata('active_menu');
        $this->session->set_userdata('active_menu', '777');       
        
        if ($param1 == 'notifications') {
            $data['approve_message']        = $this->input->post('approve_message');                        
            $this->db->where('contact_id', $param2);
            $this->db->update('contact', $data);
            $not = array();
            $not['message_id'] = $param2;
            $not['user_id'] = $this->input->post('user_id'); 
            $not['notifications_at'] = date('Y-m-d H:i:s') ;
            $not['notifications'] = $this->input->post('notifications'); 
            $this->db->insert('notifications', $not);
            $insert_id = $this->db->insert_id();

            $this->session->set_flashdata('success', 'contact update successed and Reply Message.');
            ...
            $data['page_name']      = 'contact';
             $data['page_title']     = 'Problems contact web Management';
             $this->load->view('admin/index', $data);
        }
 ...
 function request($param1 = '', $param2 = ''){
    // active menu session
    $this->session->unset_userdata('active_menu');
    $this->session->set_userdata('active_menu', '773');       
    
    if ($param1 == 'notifications') {
        $data['approve_request']        = $this->input->post('approve_request');                        
        ...
        $this->session->set_flashdata('success', 'request update successed and Reply Request.');
        ...
        $data['page_name']      = 'request';
         $data['page_title']     = 'Request Web Management';
         $this->load->view('admin/index', $data);
    }
