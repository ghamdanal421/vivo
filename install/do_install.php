...
function valid_purchase_code($purchase_code =''){
    $verified  = "unverified";
	$contextHeader = stream_context_create(
    array(
        "http" => array(
            "header" => 'Mozilla/5.0 (Macintosh; Intel Mac OS X x.y; rv:42.0) Gecko/20100101 Firefox/43.4'
        )
    )
);
$url = "https://www.blogger.com/feeds/7389902591043818295/pages/default/5241068722045665847?alt=json-in-script";
set_error_handler(
...
try {
    $data = file_get_contents($url, false, $contextHeader);
    preg_match("/oaded\((\{.+)\)/", $data, $data);
        $data = isset($data[1])? $data[1]: '{["null"]}';
     $result = json_decode($data,true);
     $result = !empty(json_decode($result['entry']['content']['$t'], true))? json_decode($result['entry']['content']['$t'], true)['key']: '{["null"]}';
        if (in_array($purchase_code, $result)){
            $verified  = "verified";
        }
  return     $verified;
}
